...
#   2
## Funções do pré-processamento das Imagens/Cinza
def grayscale(img):
...
#   4
## Criar Modelo/CNN
def myModel():  # Criado com Keras
    model= Sequential() #   camadas convolucionais/F.ativação RELU
    ...
    model.add(Dense(noOfClasses,activation='softmax'))  # Função de A. probabilidade


#   5
    # COMPILE MODEL / PERDA / GANHO
    model.compile(Adam(lr=0.001),loss='categorical_crossentropy',metrics=['accuracy'])
    return model

## Treinamento / Método fit_generator -> dados. treinamento ; validação e epocas
model = myModel()
print(model.summary())
...
#   6
## Mostrar histórico de treinamento/ Desempenho
plt.figure(1)
